    ...
    VkDescriptorSetLayout layoutPipeline)
{
    ...
    auto vulkanObjectBuffers = vulkanUtils->getObjectBuffers();
    const uint matrixBufferSize = sizeof(Matrix4x4) * VulkanObjectBuffers::AMOUNT_OF_OBJECTS;
    ...
    bufferBonesInfo.buffer = vulkanObjectBuffers->getBonesBuffer(frameNum);
    ...
    writes[1].dstSet = frames[frameNum].descriptorSetTextured;
    ...
    writes[2].dstSet = frames[frameNum].descriptorSetTextured;
